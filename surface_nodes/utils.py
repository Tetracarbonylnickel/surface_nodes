...
class PlotProperties(base.ProcessAtoms):
    ...
    def run(self):
        ...
        self.plot_structure_property([frame.calc.results["energy"] for frame in frames], 'energy', self.energy_img)
        self.plot_structure_property(
            [frame.calc.results["energy_uncertainty"] for frame in frames],
            'energy_uncertainty',
            self.energy_uncertainty_img
        )
        
        self.plot_cartesian_property([frame.calc.results["forces"] for frame in frames], 'forces', self.dim_reduction, self.forces_img)
        self.plot_cartesian_property(
    ...
    def plot_cartesian_property(self, properties, label, reduction, image):
        
        properties = self.pad_list(properties)
